...
static bool	solve_cone_side(t_ray ray, t_cone cone, double *t_side)
{
    ...
    abc[0] = vec3_dot(ray.direction, ray.direction)
        - (1.0 + tan2) * (m1 * m1);
    abc[1] = 2.0 * (vec3_dot(ray.direction, oc)
        - (1.0 + tan2) * m1 * m2);
    abc[2] = vec3_dot(oc, oc)
        - (1.0 + tan2) * (m2 * m2);
...
static void	fill_hit_cone(t_ray ray, t_hit *hit, t_cone cone,
    double t_final, double t_side)
{
    t_vec3	axis;
    t_vec3	p;
    double	h;
    ...
    {
        hit->normal = vec3_mul(axis, (vec3_dot(ray.direction, axis) < 0.0)
            ? 1.0 : -1.0);
    }
