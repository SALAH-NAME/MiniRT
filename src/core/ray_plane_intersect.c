...
/*   Updated: 2025/02/23 23:08:08 by ysemlali         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "core.h"

static t_vec3	get_denom(t_ray ray, t_plane plane, double *denom)
{
	double	rotation_matrix[9];
	t_vec3	transformed_normal;

	matrix3_create_rotation(rotation_matrix, plane.normal);
	transformed_normal = vec3_normalize(matrix3_apply_rotation(rotation_matrix,
				plane.normal));
...
bool	ray_plane_intersect(t_ray ray, t_object *obj, t_hit *hit)
{
	...
	transformed_normal = get_denom(ray, plane, &denom);
